...
nta_air = 1.0003
A = np.array([50,50,3.5]) #bed dimentions(cm)
...
#direction cosines of the ray
    l = random.choice(flat_list) #select a random penetration length from the data
...
#code for checking whether to do reflection or refraction
            ...
            if rand1< rho_avg: # spectral reflection
                dot_prod = np.dot(normal_dir_cos,Dir_cosines[ii])
                reflected_dir_cos1 = np.array([(Dir_cosines[ii][0] - 2*normal_dir_cos[0]*dot_prod), (Dir_cosines[ii][1] - 2*normal_dir_cos[1]*dot_prod), (Dir_cosines[ii][2] - 2*normal_dir_cos[2]*dot_prod)])
                ...
                # adding the latest direction cosines and the end point to the list
                x = np.vstack((x,ep))
